...
export interface DocumentEngineOptions {
	...
	): CompatibilityEvaluation | CompatibilityDiagnostic | null;
	marker?: string;
...
export function evaluateDocument(
	...
	const documentDiagnostics: Array<
	...
	const incorrectMarkers = (options.incorrectMarkers ?? []).filter(
	...
	for (const span of scanInlineCode(sourceText)) {
		...
		if (!trimmedContent.startsWith(marker)) {
			const incorrectMarker = incorrectMarkers.find((candidate) =>
				trimmedContent.startsWith(candidate),
			);
			if (incorrectMarker) {
				const staleExpression = trimmedContent
					.slice(incorrectMarker.length)
					.trim();
				const staleDeclaration = parseDeclaration(
				...
				if (staleDeclaration.kind === "success") {
					...
					unboundDeclarations.set(
						inline.declaration.normalizedLabel,
						inline.declaration.label,
					);
					add(
						inline,
						diagnostic(
							"incorrect-marker",
							`Incorrect Quantities marker "${incorrectMarker}". This vault uses "${marker}".`,
							from,
						),
					);
					continue;
		...
		const structural = !inline.declaration
			? parseStructural(expressionSource)
			: undefined;
		if (structural) {
			const structuralError = applyStructural(structural, context, from);
			if (structuralError) {
				documentDiagnostics.push({
					...structuralError.diagnostic,
					severity: "error",
				});
		...
		if (parsedExpression.kind === "error") {
			...
			localGroupsToClose =
				parsedExpression.node.scope === "local"
					? [parsedExpression.node.group]
					: [];
			if (
				inline.declaration &&
				aggregateConflictsWithDeclaration(
					parsedExpression.node,
					inline.declaration,
				) &&
				...
				outcome = diagnostic(
					...
					"An aggregate result cannot join the group it is currently aggregating.",
					from,
				);
			}
		} else {
			...
			if (
				inline.declaration &&
				!context.variables.has(inline.declaration.normalizedLabel) &&
				referencesLabel(expressionSource, inline.declaration.normalizedLabel)
			) {
				outcome = diagnostic(
					"circular-reference",
					`Declaration "${inline.declaration.label}" cannot reference itself before it has a value.`,
					from,
				);
				add(inline, outcome);
				unboundDeclarations.set(
			...
			if (reserved) {
				outcome = diagnostic(
					"reserved-identifier",
					`"${reserved}" is reserved for regional structure.`,
					from,
				);
			} else {
				const scalar = evaluateScalar(
					parsedExpression.node,
					context,
					options,
					from,
				...
				const unbound =
					scalar.outcome.kind === "error" &&
					scalar.outcome.diagnostic.code === "unknown-variable"
						? Array.from(unboundDeclarations.entries()).find(([name]) =>
								referencesLabel(expressionSource, name),
							)
						: undefined;
				outcome = unbound
					? diagnostic(
							"unbound-variable",
							`Variable "${unbound[1]}" is declared but has no calculated value.`,
							from,
						)
					: scalar.outcome;
		...
		if (inline.declaration && parsedExpression.kind === "success") {
			const conflicts =
				parsedExpression.node.kind === "scalar"
					? parsedExpression.node.aggregates.some((item) =>
							aggregateConflictsWithDeclaration(item.node, inline.declaration!),
						)
					: false;
			if (conflicts && outcome.kind === "success") {
				outcome = diagnostic(
					"aggregate-group-membership",
					"An aggregate result cannot join a group it is currently aggregating.",
					from,
				);
			}
		}
		...
		if (outcome.kind === "success") {
			for (const group of Array.from(new Set(localGroupsToClose))) {
				completeLocalAccumulation(
					group,
					context,
					from,
					inline.declaration
						? `declaration@${inline.declaration.sourceOffset}`
						: undefined,
				);
			}
		}
		...
		if (inline.declaration) {
			if (outcome.kind === "success") {
				const rootAggregateFunction =
					parsedExpression.kind === "success" &&
					parsedExpression.node.kind === "aggregate"
						? parsedExpression.node.function
						: undefined;
				const aggregateResultCollection =
					parsedExpression.kind === "success" &&
					parsedExpression.node.kind === "aggregate" &&
					parsedExpression.node.resultCollection;
				const member = declare(
					inline.declaration,
					outcome.value,
					context,
					containsAggregate,
				...
				if (context.regional.active)
					context.regional.active.members.push(member);
...
function parseStructural(source: string): StructuralNode | undefined {
	...
	if (namedTop) {
		...
		if (name && !RESERVED_NAMES.has(name))
			return { kind: "region-top", name, nameSource };
	}
...
function applyStructural(
	...
	if (node.kind === "region-top") {
		if (context.regional.active)
			return diagnostic(
				"region-already-open",
				'A region is already active; close it with "bottom" before opening another.',
				offset,
			);
		if (node.name && context.regional.usedNames.has(node.name))
			return diagnostic(
				"duplicate-region-name",
				`Region "${node.nameSource}" has already been used.`,
				offset,
			);
		const ordinal = context.regional.nextOrdinal++;
		context.regional.active = {
			id: `region:${ordinal}`,
			ordinal,
			status: "active",
			members: [],
	...
	if (!context.regional.active)
		return diagnostic("region-not-open", "No region is active.", offset);
	...
	if (context.regional.active.name)
		context.regional.namedCompleted.set(
			context.regional.active.name,
			context.regional.active,
		);
...
function evaluateScalar(
	...
	options: DocumentEngineOptions,
	offset: number,
): { outcome: EvaluationOutcome; localGroups: string[] } {
	...
	for (let index = node.aggregates.length - 1; index >= 0; index--) {
		...
		const aggregate = evaluateAggregate(
			{
				kind: "aggregate",
				function: occurrence.node.fn,
				scope: occurrence.node.scope,
				group: occurrence.node.group,
				groupSource: occurrence.node.groupSource,
				filters: occurrence.node.filters,
				resultCollection: occurrence.node.resultCollection,
			},
			context,
			options,
			offset,
		...
		if (aggregate.kind !== "success")
			return { outcome: aggregate, localGroups: [] };
		if (aggregate.value.kind === "text")
			return {
				outcome: diagnostic(
					"text-arithmetic",
					"A list result cannot be embedded in arithmetic.",
					offset,
				),
				localGroups: [],
			};
		...
		if (occurrence.node.scope === "local")
			localGroups.push(occurrence.node.group);
	}

	if (!node.aggregates.length) {
		const directValue = context.variables.get(normalizeLabel(source.trim()));
		if (directValue?.kind === "text")
			return {
				outcome: {
					kind: "success",
					value: directValue,
					display: directValue.value,
				},
				localGroups,
			};
		...
		if (compatibility?.kind === "success")
			return {
				outcome: {
					...
					consumeTrailingS: compatibility.consumeTrailingS,
				},
				localGroups,
			};
		if (compatibility?.kind === "error")
			return {
				outcome: diagnostic(compatibility.code, compatibility.message, offset),
				localGroups: [],
			};
	}
	const referencedText = Array.from(context.variables.entries()).find(
		([name, value]) => value.kind === "text" && referencesLabel(source, name),
	);
	if (referencedText)
		return {
			outcome: diagnostic(
				"text-arithmetic",
				`Text variable "${context.variableLabels.get(referencedText[0]) ?? referencedText[0]}" cannot be used in arithmetic.`,
				offset,
			),
			localGroups: [],
		};
	const arithmetic = evaluateArithmetic(
		source.replace(/\s*=\s*$/, ""),
		variables,
		context.variableLabels,
	);
	if (arithmetic.kind === "error")
		return {
			outcome: diagnostic(arithmetic.code, arithmetic.message, offset),
			localGroups: [],
		};
	return {
		outcome: {
			...
			value: {
				...
				decimalPlaces: arithmetic.decimalPlaces,
			},
...
function reservedIdentifier(expression: string): string | undefined {
	...
	for (const name of RESERVED_NAMES) {
		const match = normalized.match(
			new RegExp(
				`(^|[^\\p{L}\\p{N}_])(${name})(?!\\s+\\p{L})(?=$|[^\\p{L}\\p{N}_])`,
				"u",
			),
		);
...
function aggregateConflictsWithDeclaration(
	node: AggregateExpressionNode | { scope: string; group: string },
	declaration: NonNullable<InlineSource["declaration"]>,
): boolean {
	return (
		node.scope !== "regional" &&
		[declaration.normalizedLabel, ...declaration.groups].includes(node.group)
	);
}
...
function declare(
	...
	rootAggregateFunction?: AggregateExpressionNode["function"],
	aggregateResultCollection = false,
): EvaluatedDeclaration {
	const declarationId = `declaration@${declaration.sourceOffset}`;
	const provenance = {
		declarationIds: Array.from(
			new Set([...(value.provenance?.declarationIds ?? []), declarationId]),
		),
		sourceOffsets: Array.from(
			new Set([
				...(value.provenance?.sourceOffsets ?? []),
				declaration.sourceOffset,
			]),
		),
	...
	const memberships = Array.from(
		new Set([declaration.normalizedLabel, ...declaration.groups]),
	);
...
function updateLocalAccumulations(
	member: EvaluatedDeclaration,
	context: EvaluationContext,
) {
	for (const group of member.groups) {
		...
		if (
			provenance &&
			!provenance.localAccumulationIds.includes(accumulation.id)
		)
			provenance.localAccumulationIds.push(accumulation.id);
	}
}

function completeLocalAccumulation(
	group: string,
	context: EvaluationContext,
	offset: number,
...
function completeAllLocalAccumulations(
	context: EvaluationContext,
	offset: number,
) {
	for (const group of Array.from(context.local.active.keys()))
		completeLocalAccumulation(group, context, offset);
}
...
function evaluateAggregate(
	...
	if (node.scope === "regional") {
		...
		if (node.group) {
			...
			if (!region) {
				if (context.regional.active?.name === node.group)
					return diagnostic(
						"region-not-completed",
						`Region "${node.groupSource}" is still active; use ":>" until it is closed.`,
						offset,
					);
				return diagnostic(
					"unknown-region",
					`Unknown completed region "${node.groupSource}".`,
					offset,
				);
			}
		} else {
			...
			if (!region)
				return diagnostic(
					"unknown-region",
					"No active or completed region is available.",
					offset,
				);
		}
		const candidates = node.resultCollection
			? region.members.filter(
					(member) =>
						member.rootAggregateFunction === node.function &&
						!member.aggregateResultCollection,
				)
	...
	if (node.scope === "local") {
		...
		if (!accumulation)
			return diagnostic(
				"unknown-local-group",
				`Unknown local group "${node.groupSource}".`,
				offset,
			);
	...
	if (!context.groups.has(node.group))
		return diagnostic(
			"unknown-group",
			`Unknown group "${node.groupSource}".`,
			offset,
		);
	return aggregateMembers(
		node,
		context.groups.get(node.group) ?? [],
		options,
		offset,
	);
}
...
function aggregateMembers(
	node: AggregateExpressionNode,
	initialMembers: EvaluatedDeclaration[],
	options: DocumentEngineOptions,
	offset: number,
): EvaluationOutcome {
	let members = Array.from(
		new Map(
			initialMembers.map((member) => [member.declarationId, member]),
		).values(),
	);
	members = members.filter((member) =>
		node.filters.every((filter) => {
			const matches = filter.sigils.some((sigil) =>
				member.groups.includes(sigil),
			);
	...
	if (node.function === "list") {
		...
		return {
			...
			value: {
				kind: "text",
				value,
				provenance: memberProvenance(members, node),
			},
			display: value,
		};
	}
	...
	if (text)
		return diagnostic(
			"text-aggregate",
			`Numeric aggregate contains text declaration "${text.label}".`,
			offset,
		);
	...
	if (quantity)
		return diagnostic(
			"quantity-aggregate",
			`Aggregate contains unit-bearing declaration "${quantity.label}".`,
			offset,
		);
	const values = members.map(
		(member) =>
			new ExactDecimal(
				member.value.kind === "text"
					? 0
					: (member.value.exact ?? member.value.value.toString()),
			),
	);
	const decimalPlaces =
		node.function === "count"
			...
			: members.reduce(
					(maximum, member) =>
						member.value.kind === "number"
							? Math.max(maximum, member.value.decimalPlaces)
							: maximum,
					0,
				);
	...
	switch (node.function) {
		case "count":
			value = new ExactDecimal(values.length);
			break;
		case "sum":
			value = values.reduce((sum, item) => sum.plus(item), new ExactDecimal(0));
			break;
		case "avg":
			if (!values.length)
				return diagnostic(
					"empty-aggregate",
					"Cannot average an empty selection.",
					offset,
				);
			value = values
				.reduce((sum, item) => sum.plus(item), new ExactDecimal(0))
		...
		case "median": {
			if (!values.length)
				return diagnostic(
					"empty-aggregate",
					"Cannot take the median of an empty selection.",
					offset,
				);
		...
		case "max":
			if (!values.length)
				return diagnostic(
					"empty-aggregate",
					`Cannot take ${node.function} of an empty selection.`,
					offset,
				);
			value = values.reduce((selected, item) =>
				node.function === "min"
					? item.lessThan(selected)
						? item
						: selected
					: item.greaterThan(selected)
						? item
						: selected,
			);
			break;
	...
	return {
		...
		display: options.formatNumber(exact, decimalPlaces),
	};
}
	...
	return {
		declarationIds: Array.from(
			new Set(
				members.flatMap(
					(member) =>
						member.value.provenance?.declarationIds ?? [member.declarationId],
				),
			),
		),
		sourceOffsets: Array.from(
			new Set(
				members.flatMap(
					(member) =>
						member.value.provenance?.sourceOffsets ?? [member.sourceOffset],
				),
			),
		),
		localAccumulationIds: Array.from(
			new Set(members.flatMap((member) => member.localAccumulationIds)),
		).filter(
			(id) => node.scope !== "local" || id.startsWith(`local:${node.group}:`),
		),
	};
}

function normalizeRegionalName(value: string): string {
	const normalized = /\s/u.test(value)
		? normalizeLabel(value)
		: normalizeSigil(value);

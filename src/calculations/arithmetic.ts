...
export function evaluateArithmetic(
	...
	variableLabels: Map<string, string> = new Map(),
): ArithmeticOutcome {
	...
	if (!value.isFinite())
		return {
			...
			message:
				"The expression produced a non-finite result (possibly division by zero).",
		};
...
function resolveVariables(
	source: string,
	variables: Map<string, EvaluatedValue>,
): { expression: string; unknown?: string; decimalPlaces: number } {
	...
	const identifierSource = expression.replace(
		...
		(_match, prefix: string, literal: string) =>
			`${prefix}${" ".repeat(literal.length)}`,
	);
	const identifier = identifierSource
		.match(/[\p{L}_][\p{L}\p{N}_ ]*/u)?.[0]
		.trim();
	return identifier
		? { expression, unknown: identifier, decimalPlaces }
		: { expression, decimalPlaces };
}

function unknownVariableMessage(
	raw: string,
	variables: Map<string, EvaluatedValue>,
	...
	const accepted =
		best &&
		!tied &&
		(best.distance <= 1 || (best.distance === 2 && best.name.length >= 5));
	const suggestion =
		accepted && best
			? ` Did you mean "${variableLabels.get(best.name) ?? best.name}"?`
			: "";
...
class Parser {
	...
	private expression(): DecimalValue | null {
		let value = this.term();
		...
		while (true) {
			...
			value = op === "+" ? value.plus(rhs) : value.minus(rhs);
		}
	}
	private term(): DecimalValue | null {
		...
		while (true) {
			...
			const rhs = this.power();
			if (rhs === null) return null;
			value = op === "*" ? value.times(rhs) : value.dividedBy(rhs);
		}
	}
	private power(): DecimalValue | null {
		...
		if (rhs === null) return null;
		try {
			return value.pow(rhs);
		} catch {
			return null;
		}
	}
	private unary(): DecimalValue | null {
		this.space();
		const op = this.peek();
		if (op !== "+" && op !== "-") return this.primary();
		this.index++;
		const value = this.unary();
		return value === null ? null : op === "-" ? value.negated() : value;
	}
	private primary(): DecimalValue | null {
		this.space();
		if (this.peek() === "(") {
			this.index++;
			const value = this.expression();
	...
	private number(): DecimalValue | null {
		this.space();

...
export type AggregateParseOutcome =
	| { kind: "success"; node: AggregateExpression }
	| { kind: "none" }
...
export function parseAggregate(source: string): AggregateParseOutcome {
	...
	if (
		anyHead &&
		!(FUNCTIONS as readonly string[]).includes(anyHead[1].toLowerCase())
	) {
		return error(`Unknown aggregate function "${anyHead[1]}".`);
	}
	if (/^(?:sum|count|avg|median|min|max|list)::/i.test(trimmed))
		return error("Malformed aggregate expression.");
	for (const clause of trimmed.matchAll(/\{([^{}]*)\}/g)) {
		...
		if (negative.some(Boolean) && !negative.every(Boolean)) {
			return error(
				"Positive and negative sigils cannot be mixed in one filter clause. Use {!gr,!ls} to exclude both, or separate clauses like {ls}{!gr} to combine polarities.",
			);
		}
		if (
			terms.some((term) =>
				RESERVED.has(normalizeMembership(term.replace(/^!/, "").trim())),
			)
		) {
			return error(
				'"top" and "bottom" are reserved for regional structure.',
				"reserved-identifier",
			);
		}
	}
	const namedTarget = trimmed
		.match(
			/^(?:sum|count|avg|median|min|max|list)(?:@|:)(?!>)([^{}+\-*/^)]+)/i,
		)?.[1]
		.trim();
	...
	if (!parsed) {
		if (/^(?:sum|count|avg|median|min|max|list)(?:\b|[:@])/i.test(trimmed))
			return error("Malformed aggregate expression.");
		return { kind: "none" };
	}
	if (source.slice(parsed.to).trim())
		return error("Malformed aggregate expression.");
	return { kind: "success", node: parsed.node };
}

export function findAggregateOccurrences(
	source: string,
): AggregateOccurrence[] {
	...
	return occurrences.filter(
		(item, index) => index === 0 || item.from >= occurrences[index - 1].to,
	);
}

function parseAggregateAt(
	source: string,
	from: number,
): AggregateOccurrence | null {
	...
	const scope: AggregateScope =
		head[2] === "@" ? "local" : head[2] === ":>" ? "regional" : "global";
	...
	while (input[index] === "{") {
		...
		const sigils = terms.map((term) =>
			normalizeMembership(term.replace(/^!/, "").trim()),
		);
		if (sigils.some((sigil) => !validMembership(sigil) || RESERVED.has(sigil)))
			return null;

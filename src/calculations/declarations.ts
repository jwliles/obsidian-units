...
export function parseDeclaration(
	...
	if (!raw)
		return error(
			"invalid-declaration",
			"A declaration requires a label.",
			sourceOffset,
		);
	if (raw.includes("="))
		return error(
	...
	if (!label)
		return error(
			"invalid-label",
			"A declaration requires a non-empty label.",
			sourceOffset,
		);
	if (label.includes(":"))
		return error(
			"invalid-label",
			"Literal colons are not supported in variable labels.",
			sourceOffset,
		);
	const normalizedLabel = normalizeLabel(label);
	if (RESERVED_NAMES.has(normalizedLabel))
		return error(
			"reserved-identifier",
			`"${label}" is reserved for regional structure.`,
			sourceOffset,
		);
	...
	for (const rawSigil of parts) {
		if (rawSigil !== rawSigil.trim()) {
			return error(
				"invalid-label",
				"Literal colons are not supported in variable labels; group sigils must attach directly to the label.",
				sourceOffset,
			);
		}
		...
		if (!sigil)
			return error(
				"invalid-sigil",
				"Group sigils cannot be empty.",
				sourceOffset,
			);
		if (RESERVED_NAMES.has(sigil))
			return error(
				"reserved-identifier",
				`"${rawSigil}" is reserved for regional structure.`,
				sourceOffset,
			);
		if (
			/\s|[=:{},!|`+\-*/^()]/u.test(sigil) ||
			Array.from(sigil).length > MAX_SIGIL_LENGTH
		) {
			return error(
				"invalid-sigil",
				`Invalid group sigil "${rawSigil}".`,
				sourceOffset,
			);
		}

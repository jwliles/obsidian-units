...
function displays(source: string) {
	return outcomes(source).map((outcome) =>
		outcome.kind === "success" ? outcome.display : outcome.kind,
	);
}
...
assert.equal(
	exactCancellation.kind === "success" &&
		exactCancellation.value.kind === "number"
		? exactCancellation.value.exact
		: "error",
	"0",
);
...
if (localAst.kind === "success") {
	...
	if (localAst.node.kind === "aggregate")
		assert.equal(localAst.node.scope, "local");
}
...
if (
	aggregateContainingAst.kind === "success" &&
	aggregateContainingAst.node.kind === "scalar"
) {
...
if (
	compactAggregateContainingAst.kind === "success" &&
	compactAggregateContainingAst.node.kind === "scalar"
) {
	assert.equal(compactAggregateContainingAst.node.aggregates.length, 1);
	assert.equal(
		compactAggregateContainingAst.node.aggregates[0].node.group,
		"ex",
	);
}
...
if (mixedFilter.kind === "error")
	assert.match(mixedFilter.message, /separate clauses/);
assert.equal(parseAggregate("sum:ex{}").kind, "error");
...
if (labelHistoryAggregate.kind === "success")
	assert.equal(labelHistoryAggregate.node.group, "running total");
...
for (const fn of ["sum", "count", "avg", "median", "min", "max"]) {
	...
	if (unknownGroup.kind === "error")
		assert.equal(unknownGroup.diagnostic.code, "unknown-group");
}
const emptyFiltered = outcomes(
	"item:known = `=1`\n`=sum:known{missing}`\n`=count:known{missing}`",
);
assert.equal(
	emptyFiltered[1].kind === "success" ? emptyFiltered[1].display : "",
	"0",
);
assert.equal(
	emptyFiltered[2].kind === "success" ? emptyFiltered[2].display : "",
	"0",
);
for (const fn of ["avg", "median", "min", "max"])
	assert.equal(
		outcomes(`item:known = \`=1\`\n\`=${fn}:known{missing}\``)[1].kind,
		"error",
	);
...
if (implicitHistory.kind === "success")
	assert.equal(implicitHistory.value.value, 10);
const noSuggestion = outcomes("tiny = `=zq * 2`")[0];
assert.equal(noSuggestion.kind, "error");
if (noSuggestion.kind === "error")
	assert.doesNotMatch(noSuggestion.diagnostic.message, /Did you mean/);
assert.equal(outcomes("total:ex = `=sum:ex`")[0].kind, "error");
assert.equal(outcomes("later = `=future`\nfuture = `=2`")[0].kind, "error");

const quantityDocument = evaluateDocument(
	"length:measure = `=1 m to cm`\n`=sum:measure`",
	{
		...options,
		evaluateCompatibility: (source) =>
			source.includes(" to ")
				? {
						kind: "success",
						value: {
						...
						display: "1 meter = 100 centimeters",
					}
				: null,
	},
);
...
if (circular.kind === "error")
	assert.equal(circular.diagnostic.code, "circular-reference");
const divideByZero = outcomes("`=1 / 0`")[0];
assert.equal(divideByZero.kind, "error");
if (divideByZero.kind === "error")
	assert.equal(divideByZero.diagnostic.code, "non-finite-result");
...
for (const [expression, expected] of scientificCases) {
	assert.equal(
		successValue(`Result = \`=${expression}\``, 0),
		expected,
		expression,
	);
}
...
const failedShortName = outcomes(
	"a:t = `=5.00`\ntotal:t = `=sum:t`\nrunning total = `=sum:t`\ndoubled = `=running total * 2`",
);
assert.equal(failedShortName[3].kind, "success");
if (failedShortName[3].kind === "success")
	assert.equal(failedShortName[3].value.value, 10);

const customMarker = evaluateDocument(
	"CAC = `=200`\nTotal = `~CAC + 1`\nGood = `~2`",
	{
		...options,
		marker: "~",
...
if (customMarker[0].kind === "error")
	assert.equal(customMarker[0].diagnostic.code, "incorrect-marker");
assert.equal(customMarker[1].kind, "error");
if (customMarker[1].kind === "error")
	assert.equal(customMarker[1].diagnostic.code, "unbound-variable");
...
const structuralDocument = evaluateDocument(
	"`=Top`\nA = `=2`\n`=BOTTOM`\n`=sum:>`",
	options,
);
assert.deepEqual(
	structuralDocument.structures().map((entry) => entry.node.kind),
	["region-top", "region-bottom"],
);
const structuralResult = structuralDocument.entries()[3].outcome;
assert.equal(
	structuralResult.kind === "success" ? structuralResult.value.value : NaN,
	2,
);
assert.equal(
	evaluateDocument("`=top`", options).diagnostics()[0]?.code,
	"unclosed-region",
);

const namedRegions = outcomes(
	"`=top:first period`\nA:x = `=10`\n`=sum:>first period`\n`=bottom`\nNamed = `=sum:>first period`\n`=top:first period`",
);
assert.equal(
	namedRegions[2].kind === "error" ? namedRegions[2].diagnostic.code : "",
	"region-not-completed",
);
assert.equal(
	namedRegions[4].kind === "success" ? namedRegions[4].value.value : NaN,
	10,
);
assert.equal(
	namedRegions[5].kind === "error" ? namedRegions[5].diagnostic.code : "",
...
assert.equal(
	collectedResults[7].kind === "success"
		? collectedResults[7].value.value
		: NaN,
	3,
);
assert.equal(
	collectedResults[8].kind === "success"
		? collectedResults[8].value.value
		: NaN,
	3,
);
...
const allCollectors = outcomes(
	[
		...
		"Collected average = `=avg:>[]`",
		"Collected median = `=median:>[]`",
		"Collected minimum = `=min:>[]`",
		"Collected maximum = `=max:>[]`",
		"`=bottom`",
	].join("\n"),
);
assert.deepEqual(
	allCollectors
		.slice(16, 22)
		.map((outcome) => (outcome.kind === "success" ? outcome.value.value : NaN)),
	[14, 2, 6, 6, 1, 10],
);

const listResults = outcomes(
	"Alpha:g = `=1`\nBeta:g = `=2`\nNames:text = `=list:g`\nCopy = `=Names`\n`=sum:text`\n`=Names + 1`",
);
assert.equal(
	listResults[2].kind === "success" ? listResults[2].display : "",
	"Alpha, Beta",
);
assert.equal(
	listResults[3].kind === "success" ? listResults[3].display : "",
	"Alpha, Beta",
);
assert.equal(
	listResults[4].kind === "error" ? listResults[4].diagnostic.code : "",
	"text-aggregate",
);
assert.equal(
	listResults[5].kind === "error" ? listResults[5].diagnostic.code : "",
	"text-arithmetic",
);
const embeddedList = outcomes("A:g = `=1`\n`=list:g + 1`")[1];
assert.equal(
	embeddedList.kind === "error" ? embeddedList.diagnostic.code : "",
	"text-arithmetic",
);
assert.equal(displays("A:g = `=1`\nEmpty = `=list:g{missing}`")[1], "");
const unknownList = outcomes("`=list:missing`")[0];
assert.equal(
	unknownList.kind === "error" ? unknownList.diagnostic.code : "",
	"unknown-group",
);
assert.equal(shouldConcealStructuralMarkerInReadingView(true), true);
assert.equal(
	shouldConcealStructuralMarkerInReadingView(true, { severity: "warning" }),
	false,
);
...
assert.equal(
	concealDeclarationSigils("Prose: remains visible"),
	"Prose: remains visible",
);

const localFunctions = outcomes(
	"A:g = `=1.00`\nB:g = `=3.00`\n`=avg@g`\n`=min@g`\n`=max@g`",
);
assert.equal(
	localFunctions[2].kind === "success" ? localFunctions[2].value.value : NaN,
	2,
);
assert.equal(
	localFunctions[3].kind === "success" ? localFunctions[3].value.value : NaN,
	1,
);
assert.equal(
	localFunctions[4].kind === "success" ? localFunctions[4].value.value : NaN,
	3,
);
if (localFunctions[2].kind === "success") {
	assert.deepEqual(localFunctions[2].value.provenance?.localAccumulationIds, [
		"local:g:1",
	]);
}

...
for (const file of fixtureFiles) {
	...
	const index = evaluateDocument(source, {
		...
		evaluateCompatibility: (expression, values) =>
			evaluateCompatibilityExpression(expression, values, DEFAULT_SETTINGS),
	});
	const structures = new Map(
		index.structures().map((entry) => [entry.source.from, entry.node.kind]),
	);
	const diagnostics = new Map(
		index.diagnostics().map((item) => [item.offset, item]),
	);
	...
	for (const entry of index.entries()) {
		...
		const commentSource = followingLines[0].includes("<!--")
			? followingLines[0]
			: followingLines[1]?.trimStart().startsWith("<!--")
				? followingLines[1]
				: "";
		const expected = commentSource.match(
			/<!--\s*expect:(value|empty|error|structure|warning)(?:\s+(.+?))?\s*-->/i,
		);
		...
		if (expectation === "value" || expectation === "empty") {
			...
			if (entry.outcome.kind !== "success") {
				failures.push(
					`${description}: expected ${expectation === "empty" ? "an empty value" : `value ${expectedDisplay}`}, got ${describeOutcome(entry.outcome)}`,
				);
			} else if (entry.outcome.display !== expectedDisplay) {
				failures.push(
					`${description}: expected value ${expectedDisplay || "<empty>"}, got ${entry.outcome.display || "<empty>"}`,
				);
			}
			...
			if (actual !== expected[2])
				failures.push(
					`${description}: expected structure ${expected[2]}, got ${actual ?? "none"}`,
				);
			...
			if (
				...
				actual.code !== expected[2]
			) {
				failures.push(
					`${description}: expected warning ${expected[2]}, got ${actual ? `${actual.severity} ${actual.code}` : "none"}`,
				);
			}
		} else if (entry.outcome.kind !== "error") {
			failures.push(
				`${description}: expected error ${expected[2]}, got ${describeOutcome(entry.outcome)}`,
			);
		} else if (entry.outcome.diagnostic.code !== expected[2]) {
			failures.push(
				`${description}: expected error ${expected[2]}, got ${entry.outcome.diagnostic.code}`,
			);
		}
	}
...
assert.ok(
	assertions >= 200,
	`Expected at least 200 fixture assertions, found ${assertions}`,
);
if (failures.length) {
...
console.log(
	`All Markdown fixtures passed (${assertions} assertions across ${fixtureFiles.length} files).`,
);
...
function describeOutcome(
	outcome: ReturnType<ReturnType<typeof evaluateDocument>["outcomeAt"]>,
): string {

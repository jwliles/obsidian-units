...
import { DEFAULT_SETTINGS, diagnoseConversion, evaluateCompatibilityExpression, evaluateInlineExpressionFallback, formatNumber, getRenderSource, isValidExpressionMarker, parseConversion, planMarkerUpdate } from '../main';
...
assert.equal(evaluateInlineExpressionFallback('1 ang to ly | value', lowPrecisionSettings, new Map(), true)!.text, '1.06 × 10⁻²⁶');
assert.equal(evaluateInlineExpressionFallback('18 km/h to m/s | value', lowPrecisionSettings, new Map(), true)!.text, '5');

// Density conversion in both directions.
const densitySettings = { ...settings, densities: [{ name: 'maple syrup', value: 1.37, unit: 'g/ml' }] };
assert.equal(evaluateInlineExpressionFallback('1 tsp of maple syrup to g | value', densitySettings, new Map(), true)!.text, '6.7526');
assert.equal(evaluateInlineExpressionFallback('100 g of maple syrup to ml | value', densitySettings, new Map(), true)!.text, '72.9927');
...
async function testEmbeddedSourceResolution() {
	...
	const renderSource = await getRenderSource({
		...
		app: {
			...
			metadataCache: {
				...
				getFileCache: () => ({ subpaths: { '#Report': { start: { offset: reportStart }, end: { offset: reportEnd } } } }),
			},
		},
	} as any);
	assert.deepEqual(renderSource, { text: embeddedText, from: reportStart, to: reportEnd });
	const hostFile = Object.assign(new TFile(), { path: 'tests/embed-host.md' });
	const hostText = readFileSync('tests/embed-host.md', 'utf8');
	const hostEmbedLine = hostText.split('\n').findIndex((line) => line.includes('![[embed-source#Report]]'));
	const detachedRenderSource = await getRenderSource({
		...
		getSectionInfo: () => ({ text: '![[embed-source#Report]]', lineStart: hostEmbedLine, lineEnd: hostEmbedLine }),
	} as any, {
		app: {
			vault: {
				getAbstractFileByPath: () => hostFile,
				cachedRead: async (file: TFile) => file.path === hostFile.path ? hostText : embeddedText,
			},
			metadataCache: {
				getFirstLinkpathDest: () => embeddedFile,
				getFileCache: () => ({ subpaths: { '#Report': { start: { offset: reportStart }, end: { offset: reportEnd } } } }),
	...
	assert.deepEqual(detachedRenderSource, { text: embeddedText, from: reportStart, to: reportEnd });
	const embeddedIndex = evaluateDocument(renderSource!.text, {
		marker: '=',
		formatNumber: (value, minimum) => formatNumber(value, 4, minimum),
		evaluateCompatibility: (expression, values) => evaluateCompatibilityExpression(expression, values, settings),
	});
	const reportEntries = embeddedIndex.entries().filter((entry) => entry.source.from >= renderSource!.from && entry.source.from < renderSource!.to);
	const embeddedResult = reportEntries.find((entry) => entry.source.content === '=Seed+1')?.outcome;
	const regionalResult = reportEntries.find((entry) => entry.source.content === '=sum:>')?.outcome;
	assert.equal(embeddedResult?.kind, 'success');
	assert.equal(regionalResult?.kind, 'success');
	if (embeddedResult?.kind === 'success') assert.equal(embeddedResult.display, '11');
	if (regionalResult?.kind === 'success') assert.equal(regionalResult.display, '2');
	assert.ok(!reportEntries.some((entry) => entry.source.content === '=20'));
	assert.match(readFileSync('tests/embed-host.md', 'utf8'), /!\[\[embed-source#Report\]\]/);
}

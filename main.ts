...
const UNIT_DISPLAYS: Record<string, UnitDisplay> = {
	...
	"cm/s^2": {
		abbr: "cm/s^2",
		singular: "centimeter per second squared",
	...
	"km/h/s": {
		abbr: "km/h/s",
		singular: "kilometer per hour per second",
	...
	"mph/s": {
		abbr: "mph/s",
		singular: "mile per hour per second",
...
export default class ObsidianQuantitiesPlugin extends Plugin {
	...
	async onload() {
		...
		this.registerEditorExtension([
			...
			Prec.highest(createObsidianQuantitiesLivePreviewExtension(this)),
		]);
		this.registerMarkdownPostProcessor((element, ctx) =>
			renderInlineCodeConversions(element, ctx, this),
		);
	}
	...
	evaluateInlineExpression(
		...
		return evaluateInlineExpressionFallback(
			text,
			this.settings,
			variables,
			isExplicit,
	...
	evaluateDocument(source: string): DocumentEvaluationIndex {
		const settingsKey = JSON.stringify([
			...
			this.settings.expressionMarker,
			this.settings.previousExpressionMarker,
			this.settings.unitDisplayOverrides,
			this.settings.densities,
		]);
		if (
			this.evaluationCache?.source === source &&
			this.evaluationCache.settingsKey === settingsKey
		) {
			return this.evaluationCache.index;
		}
		const index = evaluateDocument(source, {
			formatNumber: (value, minimumDecimalPlaces = 0) =>
				formatNumber(value, this.settings.precision, minimumDecimalPlaces),
			evaluateCompatibility: (expression, values) =>
				evaluateCompatibilityExpression(expression, values, this.settings),
			marker: this.settings.expressionMarker,
			incorrectMarkers: [
				this.settings.previousExpressionMarker,
				DEFAULT_SETTINGS.expressionMarker,
			],
		});
	...
	private updateMarkersInCurrentFile(editor: Editor) {
		...
		if (update.changed === 0) {
			new Notice(
				`No recognized Quantities expressions use the marker "${from}". ${update.ambiguous} ambiguous span(s) were left unchanged.`,
			);
			return;
		}
		new MarkerUpdateConfirmModal(this.app, from, to, update, () => {
			...
			new Notice(
				`Updated ${update.changed} Quantities marker(s). ${update.ambiguous} ambiguous span(s) were left unchanged.`,
			);
		}).open();
	}
	...
	isLivePreviewRenderingEnabled(view: EditorView): boolean {
		return (
			view.state.field(this.livePreviewRenderingField, false)?.enabled ??
			this.settings.renderInLivePreviewByDefault
		);
	}

	livePreviewRenderingGeneration(view: EditorView): number {
		return (
			view.state.field(this.livePreviewRenderingField, false)?.generation ?? 0
		);
	}

	livePreviewRenderingChanged(update: ViewUpdate): boolean {
		return (
			update.startState.field(this.livePreviewRenderingField, false) !==
			update.state.field(this.livePreviewRenderingField, false)
		);
	}
	...
	private convertEditorExpression(editor: Editor) {
		...
		if (!conversion) {
			new Notice(
				diagnoseConversion(range.text, this.settings) ??
					'Could not find a unit conversion like "5 ft to cm".',
			);
			return;
		}
		...
		editor.setCursor({
			...
			ch: range.from.ch + replacement.length,
		});
	}
	...
	private insertResultOnly(editor: Editor) {
		...
		if (matches.length > 0) {
			...
			for (const match of matches) {
				...
				const commandEvaluation =
					outcome?.kind === "not-applicable"
						? this.evaluateInlineExpression(
								match[1],
								collectVariables(prefixText, this.settings),
								true,
							)
						: null;
			...
			if (!evaluatedAny) {
				new Notice(
					firstFailure ?? "No inline expression to evaluate on this line.",
				);
				return;
			}
		...
		const evaluation = this.evaluateInlineExpression(
			trimmed,
			variables,
			isExplicit,
		);
...
function createLivePreviewRenderingField(
	plugin: ObsidianQuantitiesPlugin,
): StateField<LivePreviewRenderingState> {
...
const CONVERSION_PATTERN =
	/^(-?(?:\d+(?:\.\d*)?|\.\d+)(?:e[+-]?\d+)?)\s*(.+?)(?:\s+of\s+(.+?))?\s*(?:->|\bto\b|\bas\b|\bin\b)\s*(.+)$/i;

export function parseConversion(
	text: string,
	settings: ObsidianQuantitiesSettings,
): Conversion | null {
	const cleaned = stripArithmeticEquals(
		stripExistingResult(stripInlineCodeMarkers(text.trim())),
	);
	...
	try {
		input = new ExactDecimal(match[1]);
	} catch {
		return null;
	}
...
export function diagnoseConversion(
	text: string,
	settings: ObsidianQuantitiesSettings,
): string | null {
	const cleaned = stripArithmeticEquals(
...
function describeEvaluationFailure(
...
function parseInlineConversion(
	text: string,
	settings: ObsidianQuantitiesSettings,
...
function parseInlineRenderMode(text: string): {
	expression: string;
	mode: InlineRenderMode;
} {
...
export function evaluateInlineExpressionFallback(
	text: string,
	settings: ObsidianQuantitiesSettings,
	variables: VariableMap = new Map(),
	isExplicit = false,
...
export function evaluateCompatibilityExpression(
	expression: string,
	...
	if (conversion) {
		...
		return {
			...
			value: {
				...
				value: conversion.conversion.output.toNumber(),
				exact: conversion.conversion.output.toString(),
				unit: conversion.conversion.target.canonical,
				dimension: conversion.conversion.target.dimension,
			},
			display,
	...
	if (numericDisplay !== null) {
		...
		return {
			...
			value: {
				...
				value: numericValue.toNumber(),
				exact: numericValue.isZero() ? "0" : numericValue.toString(),
				decimalPlaces: decimalPlacesInLiteral(expression),
			},
	...
	return message
		? { kind: "error" as const, code: "conversion-error", message }
		: null;
}

function decimalPlacesInLiteral(expression: string): number {
	const match = stripArithmeticEquals(expression).match(
		/^[+-]?(?:\d+(?:\.(\d*))?|\.(\d+))(?:e([+-]?\d+))?$/i,
	);
...
function densityToBase(density: DensityEntry): DecimalValue | null {
	...
	return new ExactDecimal(density.value.toString())
		.times(unitFactor(massUnit))
		.dividedBy(unitFactor(volUnit));
}
...
function convertWithDensity(
	...
	if (
		!densityGramsPerLiter?.isFinite() ||
		densityGramsPerLiter.lessThanOrEqualTo(0)
	) {
		return new ExactDecimal(NaN);
	}
...
function formatInlineConversion(
	inlineConversion: InlineConversion,
	settings: ObsidianQuantitiesSettings,
): string {
...
function resolveUnitDisplayForm(
	canonical: string,
	settings: ObsidianQuantitiesSettings,
): UnitDisplayForm {
...
export function formatNumber(
	...
	const decimal =
		value instanceof ExactDecimal ? value : new ExactDecimal(value);
	...
	if (
		decimal.toDecimalPlaces(precision, ExactDecimal.ROUND_HALF_EVEN).isZero()
	) {
	...
	const formatted = decimal.toFixed(
		effectivePrecision,
		ExactDecimal.ROUND_HALF_EVEN,
	);
	let compact = formatted.replace(/(\.\d*?)0+$/, "$1").replace(/\.$/, "");
	const minimum = Math.min(
		Math.max(0, minimumDecimalPlaces),
		effectivePrecision,
	);
...
function formatScientific(value: DecimalValue, precision: number): string {
	const [rawMantissa, rawExponent] = value
		.toExponential(precision, ExactDecimal.ROUND_HALF_EVEN)
		.split("e");
...
function formatNumericLiteral(text: string, precision: number): string | null {
	...
	try {
		value = new ExactDecimal(match[1]);
	} catch {
		return null;
	}
...
function parseArithmetic(
	...
	const expression = resolveVariablesInExpression(
		stripArithmeticEquals(stripInlineCodeMarkers(text.trim())),
		variables,
	);
...
function resolveVariablesInExpression(
	...
	const variableEntries = Array.from(variables.entries()).sort(
	...
	for (const [name, value] of variableEntries) {
		...
		resolved = resolved.replace(
			regex,
			(_match, prefix) => `${prefix}(${formatNumberForExpression(value)})`,
		);
	}
...
class ArithmeticParser {
	...
	private parseNumber(): DecimalValue | null {
		...
		try {
			return new ExactDecimal(this.input.slice(start, this.index));
		} catch {
			return null;
		}
	}
...
function createObsidianQuantitiesLivePreviewExtension(
	plugin: ObsidianQuantitiesPlugin,
) {
		...
		class {
			...
			update(update: ViewUpdate) {
				...
				if (
					...
					update.selectionSet ||
					livePreviewChanged ||
					plugin.livePreviewRenderingChanged(update)
				) {
...
function collectVariables(
	...
	for (const line of text.split("\n")) {
		...
		while ((match = inlineCodePattern.exec(line)) !== null) {
			...
			const inlineEvaluation = evaluateInlineExpressionFallback(
				expression,
				settings,
				variables,
				true,
...
function extractLeadingDecimal(text: string): DecimalValue | null {
	...
	const scientific = trimmed.match(
		/^(-?(?:\d+(?:\.\d*)?|\.\d+))\s*×\s*10([⁻⁰¹²³⁴⁵⁶⁷⁸⁹]+)/u,
	);
	...
	for (const range of view.visibleRanges) {
		...
		for (
			...
			while ((match = inlineCodePattern.exec(line.text)) !== null) {
				...
				if (
					selectionRanges.some((s) => s.from <= basePos && s.to >= baseFrom)
				) {
					continue;
				}

				if (outcome?.kind === "success") {
					const shouldConsumeTrailingS =
						outcome.consumeTrailingS === true &&
						line.text.charAt(localTo) === "s";
					...
					decorations.push(
						Decoration.replace({
							widget: new ObsidianQuantitiesResultWidget(
								outcome.display,
								generation,
							),
						}).range(baseFrom, pos),
				...
				if (outcome?.kind === "error") {
					decorations.push(
						Decoration.replace({
							widget: new ObsidianQuantitiesErrorWidget(
								outcome.diagnostic.message,
								generation,
							),
				...
				if (documentDiagnostic?.severity === "warning") {
					decorations.push(
						Decoration.replace({
							widget: new ObsidianQuantitiesWarningWidget(
								documentDiagnostic.message,
								generation,
							),
						}).range(baseFrom, basePos),
					);
...
async function renderInlineCodeConversions(
	element: HTMLElement,
	ctx: MarkdownPostProcessorContext,
	plugin: ObsidianQuantitiesPlugin,
) {
	const codeElements = Array.from(element.querySelectorAll("code")).filter(
		(codeElement) => !codeElement.closest("pre"),
	);
	...
	const sectionEntries =
		evaluationIndex && sectionStartOffset !== null && sectionEndOffset !== null
			? evaluationIndex
					...
					.filter(
						({ source }) =>
							source.from >= sectionStartOffset &&
							source.from < sectionEndOffset,
					)
			: [];
	const documentDiagnostics = new Map(
		(evaluationIndex?.diagnostics() ?? []).map((item) => [item.offset, item]),
	);
	const structuralOffsets = new Set(
		(evaluationIndex?.structures() ?? []).map((item) => item.source.from),
	);
	let entryCursor = 0;

	for (const codeElement of codeElements) {
		const codeText = codeElement.textContent ?? "";
		const entryIndex = sectionEntries.findIndex(
			(entry, index) =>
				index >= entryCursor && entry.source.content === codeText,
		);
		if (entryIndex < 0) continue;
		entryCursor = entryIndex + 1;
		const outcome = sectionEntries[entryIndex].outcome;
		const documentDiagnostic = documentDiagnostics.get(
			sectionEntries[entryIndex].source.from,
		);
		...
		if (documentDiagnostic?.severity === "warning") {
			codeElement.replaceWith(
				createResultSpan(
					documentDiagnostic.message,
					"obsidian-quantities-warning",
				),
			);
			continue;
		}
		if (
			shouldConcealStructuralMarkerInReadingView(
				structuralOffsets.has(sectionEntries[entryIndex].source.from),
				documentDiagnostic,
			)
		) {
		...
		const result = createResultSpan(
			outcome.display,
			"obsidian-quantities-result",
		);
		codeElement.replaceWith(result);
	}
}

export function shouldConcealStructuralMarkerInReadingView(
...
function concealDeclarationSigilsBefore(codeElement: HTMLElement): void {
	...
	if (previous?.nodeType !== Node.TEXT_NODE || previous.textContent === null)
		return;
...
function concealStructuralMarker(codeElement: HTMLElement): void {
	...
	while (
		...
		!host.querySelector("img, video, audio, iframe, canvas, svg")
	) {
...
export async function getRenderSource(
	element: HTMLElement,
	ctx: MarkdownPostProcessorContext,
	plugin: ObsidianQuantitiesPlugin,
): Promise<RenderSource | null> {
	const embedSelector = ".internal-embed[src], .internal-embed[data-src]";
	const embed = element.matches(embedSelector)
		? element
		: element.closest<HTMLElement>(embedSelector);
	const embedSource = (
		embed?.getAttribute("src") ?? embed?.getAttribute("data-src")
	)?.trim();
	if (embedSource) {
		const resolved = await resolveEmbeddedRenderSource(
			embedSource,
			ctx.sourcePath,
			plugin,
		);
	...
	if (sectionInfo) {
		...
		const detachedEmbedSource =
			embeddedWikiLinkSource(sectionInfo.text) ??
			embeddedWikiLinkSource(hostSectionText);
...
function consumeNextTextPrefix(element: HTMLElement, prefix: string) {
	...
	if (
		...
		!nextNode.textContent?.startsWith(prefix)
	) {
		return;
	}
...
export function planMarkerUpdate(
	...
	if (
		!isValidExpressionMarker(from) ||
		!isValidExpressionMarker(to) ||
		from === to
	) {
	...
	const text = source.replace(
		/`([^`\n]+)`/g,
		(whole, content: string, offset: number) => {
			...
			const declaration =
				extractAssignmentName(source.slice(0, offset)) !== null;
			const aggregate = parseAggregate(expression).kind === "success";
			const structural = /^(?:top|bottom)$/iu.test(
				expression.normalize("NFKC"),
			);
...
class ObsidianQuantitiesResultWidget extends WidgetType {
	...
	toDOM(): HTMLElement {
		return createResultSpan(
			this.text,
			"obsidian-quantities-result cm-obsidian-quantities-result",
		);
	}
...
class ObsidianQuantitiesErrorWidget extends WidgetType {
	...
	toDOM(): HTMLElement {
		return createResultSpan(
			this.text,
			"obsidian-quantities-error cm-obsidian-quantities-result",
		);
	}
...
class ObsidianQuantitiesWarningWidget extends WidgetType {
	...
	toDOM(): HTMLElement {
		return createResultSpan(
			this.text,
			"obsidian-quantities-warning cm-obsidian-quantities-result",
		);
	}
...
class ObsidianQuantitiesSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Normal maximum decimal places. Non-zero values may use more rather than appear as zero.",
			)
			.addSlider((slider) =>
				slider
					.setLimits(0, 10, 1)
		...
		new Setting(containerEl)
			...
			.setDesc(
				"The exact marker that identifies Quantities expressions. Changing it disables the previous marker until the file is updated with the command palette.",
			)
			.addText((text) => {
				...
				text.inputEl.addEventListener("blur", async () => {
					...
					if (!isValidExpressionMarker(marker)) {
						new Notice(
							"The Quantities marker must be 1–16 characters and cannot contain whitespace or backticks.",
						);
						text.setValue(this.plugin.settings.expressionMarker);
						return;
					}
					if (marker === this.plugin.settings.expressionMarker) return;
					this.plugin.settings.previousExpressionMarker =
						this.plugin.settings.expressionMarker;
		...
		new Setting(containerEl)
			...
			.setDesc(
				"New editor panes start with rendered inline results enabled. Use the toggle command to change only the active editor.",
			)
			.addToggle((toggle) =>
				toggle
	...
	private renderUnitDisplayOverrideSection(containerEl: HTMLElement): void {
		new Setting(containerEl)
			...
			.setDesc(
				"Search for a unit and pick whether to render its abbreviation (cm) or full name (centimeters). Defaults work for most cases — compound units stay short, simple units expand.",
			)
			.setHeading();
		...
		const renderOverrides = () => {
			...
			if (entries.length === 0) {
				overridesEl.createDiv({
					text: "No overrides yet.",
					cls: "obsidian-quantities-override-empty",
				});
				return;
			}
	...
	private renderOverrideRow(
		parent: HTMLElement,
		entry: UnitSearchEntry,
		onChange: () => void,
	): void {
		...
		const select = row.createEl("select", {
			cls: "obsidian-quantities-override-select",
		});
		...
		select.value =
			this.plugin.settings.unitDisplayOverrides[entry.canonical] ?? "";
		...
		select.addEventListener("change", async () => {
			if (select.value === "") {
				...
				this.plugin.settings.unitDisplayOverrides[entry.canonical] =
					select.value as UnitDisplayForm;
			}
			await this.plugin.saveSettings();
			this.plugin.refreshAllLivePreviews();
			onChange();
	...
	private renderOverrideListItem(
		parent: HTMLElement,
		canonical: string,
		form: UnitDisplayForm,
		onChange: () => void,
	): void {
		...
		const revertBtn = row.createEl("button", {
			text: "Revert",
			cls: "obsidian-quantities-override-revert",
		});
	...
	private renderDensitiesSection(containerEl: HTMLElement): void {
		new Setting(containerEl)
			...
			.setDesc(
				"Densities let the plugin convert between mass and volume for specific materials (e.g. `1 tsp of maple syrup to g`). Densities are user-defined — see the README for the data format.",
			)
			.setHeading();

		const listEl = containerEl.createDiv({
			cls: "obsidian-quantities-density-list",
		});
		...
		const renderList = () => {
			...
			this.plugin.settings.densities.forEach((entry, index) => {
				const row = listEl.createDiv({
					cls: "obsidian-quantities-override-row",
				});
				row.createSpan({
...
class DensityModal extends Modal {
	...
	onOpen(): void {
		...
		contentEl.createEl("h3", {
			text: this.name ? "Edit density" : "Add density",
		});

		new Setting(contentEl)
			.setName("Material name")
			.setDesc(
				'How you will refer to this material in expressions, e.g. "maple syrup".',
			)
			.addText((text) =>
				text
		...
		new Setting(contentEl).setName("Density value").addText((text) =>
		...
		new Setting(contentEl).setName("Density unit").addDropdown((dropdown) => {
